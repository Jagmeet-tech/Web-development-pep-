//Asynchronous behaviour -means all chrome tabs run simulataneously/parallely.. 
...
async function getIssues(url,i,j) {
    ...
    if(projectsAdded == totalProjects) {
        fs.writeFileSync("finalData2.json", JSON.stringify(finalData));
    }
    browser.close();
}
...
async function getProjects(url, i) {
    ...
    totalProjects += (projectBoxes.length > 3 ? 3 : projectBoxes.length);
